data "cloudflare_zone" "struble_dev" {
  name = "struble.dev"
}
...
resource "cloudflare_record" "spf" {
  zone_id = data.cloudflare_zone.struble_dev.id
  name    = "struble.dev"
  type    = "TXT"
...
data "cloudflare_zone" "struble_app" {
  name = "struble.app"
}

resource "cloudflare_record" "apps_cname" {
  zone_id = data.cloudflare_zone.struble_app.id
  name    = "*"
  type    = "CNAME"
  value   = "struble.app"
  proxied = true
